...
import com.firstclass.binarySearchtree.Node;

public class TreeCompareHelper<T extends Comparable<T>> {

	public boolean compareTrees(Node<T> node1, Node<T> node2) {

		// we have to check base cases (it may be leaf node so we have to use ==)
		if((node1==null && node2==null) || (node1==null && node2!=null) || (node1!=null && node2==null)) 
            return node1==node2; 

		// if the values within the nodes are not the same we return false(trees are not
		// the same)
